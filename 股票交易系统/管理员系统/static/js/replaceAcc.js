		...
		switch($(this).attr("name"))
		{
			...
			case 'optionsRadios3':
				if ($(this).attr('id') == 'optionsRadios7'){
					window.location.href = 'http://localhost/Administration/Replace/security/natual';
				}
				else if ($(this).attr('id') == 'optionsRadios2'){
					window.location.href = 'http://localhost/Administration/Replace/security/legal';
				}
				break;
		}
	});
	...
	$('#submit_Cap').click(function(){
		...
		if (state)
		{
			...
			$.ajax({
				type:"POST",
			    dataType:"json",
			    url:"/Administration/Reapply_capital/reapply_capital",
		        data:data1,
		        async:false,
	        ...
	        "Comfirm": function() {
	        	var data = "sec_acc=" + $('.form-horizontal').find('#id').val();
	          	...
	          	$.ajax({
		          	type:"POST",
			        dataType:"json",
			        url:"/Administration/Reapply_capital/confirm_reapply",
			        data:data,
			        async:false,
			        success:function(result){
			            console.log(result);
