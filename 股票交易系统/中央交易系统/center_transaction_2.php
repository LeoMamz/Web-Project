...
//$TCpostData = json_encode(array('stock_id' => '0000101041', 'stock_price' => 100, 'stock_number' => 2, 'user_id' => 123, 'date' => $date, 'type' => 0));//file_get_contents('php://input');

//$type_sell=4;
//$TCpostData=json_encode(array('user_id'=>123,'type'=>$type_sell));
//$TCpostData = json_encode(array('user_id'=>10000000000, 'stock_id'=>'0000501016', 'stock_price' => 106, 'stock_number' => 2,'type'=>1, 'date'=>'2018-06-26 12:11:25'));
$TCpostData = file_get_contents('php://input');
$TCdata = json_decode($TCpostData);
// echo json_encode($TCdata);
//$TCtype = $TCdata['type'];
$TCtype = $TCdata->type;
// echo json_encode($TCdata['stock_id']);
// var_dump($TCdata);
// 0-出售 1-购买 2-撤回出售 3-撤回购买 4-请求用户出售股票 5-请求用户购买股票 6-请求用户交易记录股票 7-查询股票是否停牌
if ( $TCtype == 7) {
    $TCstock_id = $TCdata->stock_id;
}else if(($TCtype == 4)||($TCtype == 5)||($TCtype == 6)){
...
$settlement_judge=mysqli_num_rows($settlement_result);
if(!$settlement_judge) {
    $settlement_result = mysqli_query($con, "SELECT stock_price FROM message WHERE stock_id='" . $TCstock_id . "'");
...
if ($TCtype == 2) {
    //获取该指令的状态
    $cur_delete_result = mysqli_query($con, "SELECT state FROM sell
														  WHERE stock_name 	= '" . $TCstock_id . "'
...
if ($TCtype == 3) {
    //获取该指令的状态
    $cur_delete_result = mysqli_query($con, "SELECT state FROM buy WHERE stock_name = '" . $TCstock_id . "' AND user_id  = $TCuser_id AND stock_price = $TCstock_price AND stock_num = $TCstock_number AND time = '" . $TCdate . "'");
    $if_can_delete = mysqli_fetch_array($cur_delete_result);
    //买指令可以撤回，取消该用户冻结资金
    $total_cost = $TCstock_price * $TCstock_number;
    mysqli_query($con, "UPDATE capital_acc SET active_cap = active_cap + $total_cost WHERE sec_acc = $TCuser_id");
    mysqli_query($con, "UPDATE capital_acc SET frozen_cap = frozen_cap - $total_cost WHERE sec_acc = $TCuser_id");
    //若未被处理，则撤销
    //var_dump($if_can_delete[0]);
    if ($if_can_delete[0] == 1) {
        mysqli_query($con, "DELETE FROM buy WHERE stock_name 	= '" . $TCstock_id . "'AND user_id = $TCuser_id AND stock_price = $TCstock_price AND stock_num = $TCstock_number AND time = '" . $TCdate . "'");
        $data = array('re' => 1);
        $Tcdata = json_encode($data);
        echo $Tcdata;
    } //否则不做任何变动
    ...
    if (!$row_num || !$cur_stock_state[0]) {
        ...
        if ($TCstock_price > $cur_buy[3]) {
            ...
            } else if ($TCstock_number == $cur_buy[4]) //此时买卖股数也相等
            {
                ...
                //更新该买指令状态为完全交易
                $to_sql = "UPDATE buy SET state = 3 WHERE buy_no = $cur_buy[0]";
                mysqli_query($con, $to_sql);

                //更新该卖指令状态为完全交易
                $to_sql = "UPDATE sell SET state = 3 WHERE sell_no = $sell_no_cur[0]";
                mysqli_query($con, $to_sql);
...
function price_calc($Condition, $SellPrice, $BuyPrice, $StockNumber, $sell_no, $buy_no)
{
    ...
    $buyer_id_result = mysqli_query($con, "SELECT user_id FROM buy WHERE buy_no = $buy_no");
    $seller_id_result = mysqli_query($con, "SELECT user_id FROM sell WHERE sell_no = $sell_no");

    $buyer_id = mysqli_fetch_array($buyer_id_result);
    $seller_id = mysqli_fetch_array($seller_id_result);

    //新增用户资金更新模块
    $total_cost = $Price * $StockNumber;
    $buyer_frozen_cost = $BuyPrice * $StockNumber;
    mysqli_query($con, "UPDATE capital_acc SET active_cap = active_cap + $total_cost WHERE sec_acc = $seller_id[0]");
    $frozen_cost_remain = $buyer_frozen_cost - $total_cost;
    mysqli_query($con, "UPDATE capital_acc SET frozen_cap = frozen_cap - $total_cost WHERE sec_acc = $buyer_id[0]");
    mysqli_query($con, "UPDATE capital_acc SET active_cap = active_cap + $frozen_cost_remain WHERE sec_acc = $buyer_id[0]");

  //  $managrdata = array('buy_id'=>$buyer_id[0],'sell_id'=>$seller_id[0],'stock_name'=>$stock_name,'stock_code'=>$stock_id[0],'stock_num'=>$StockNumber,'price'=>$Price);
  //  $Mdata = json_encode($managrdata);
   // $Murl = "http://localhost/Administration/Trade_interface/record";
  //  http_post_data($Murl,$Mdata );
    $stock_number_result=mysqli_query($con,"select stock_num from stock_info  where sec_acc = $seller_id[0] and stock_code = '".$stock_id[0]."'");
    $stock_number_array=mysqli_fetch_array($stock_number_result);
    $stock_num_a=$stock_number_array[0];
    mysqli_query($con,"UPDATE stock_info set stock_num = $stock_num_a - $StockNumber where sec_acc = $seller_id[0] and stock_code = '".$stock_id[0]."'");

    $stock_number_result=mysqli_query($con,"select stock_num from stock_info  where sec_acc = $buyer_id[0] and stock_code = '".$stock_id[0]."'");
    $stock_number_array=mysqli_fetch_array($stock_number_result);
    $stock_num_a=$stock_number_array[0];
    if(!$stock_num_a){
        $to_dbg_sql = "insert into stock_info values ($buyer_id[0], '" . $stock_id[0] . "', '" .$stock_name . "', $Price, $StockNumber, 200, 0)";
        mysqli_query($con, "insert into stock_info values ($buyer_id[0], '" . $stock_id[0] . "', '" .$stock_name . "', $Price, $StockNumber, 200, 0)");
    }else {
        mysqli_query($con, "UPDATE stock_info set stock_num = $stock_num_a + $StockNumber where sec_acc = $buyer_id[0] and stock_code = '" . $stock_id[0] . "'");
...
if($TCtype == 6)
{
    ...
    $sql_buy = mysqli_query($con, "select * from  buy where state = 3 and user_id = $TCuser_id");
    $sql_sell = mysqli_query($con, "select * from sell  where state = 3 and user_id = $TCuser_id");
    $datarow1 = mysqli_num_rows($sql_buy);
    $datarow2 = mysqli_num_rows($sql_sell);
    $datarow = $datarow1+$datarow2;
    for ($i = 0; $i < $datarow; $i++) {
        if($i < $datarow1){
            $sql_arr = mysqli_fetch_assoc($sql_buy);
            $stock_id = $sql_arr['stock_id'];
            $stock_price = $sql_arr['price'];
            $stock_number = $sql_arr['stock_num'];
            $date = $sql_arr['time'];
            // $user_id = $sql_arr['buy_user_id'];
            $Xflag =0;}
        else{
            $sql_arr = mysqli_fetch_assoc($sql_sell);
            $stock_id = $sql_arr['stock_id'];
            $stock_price = $sql_arr['price'];
            $stock_number = $sql_arr['stock_num'];
            $date = $sql_arr['time'];
        ...
        $data[$i] = array($stock_id, $stock_price, $stock_number, $date,$Xflag);
    }
...
//$TCtype = 0 ;
    ...
    for ($i = 0; $i < $datarow; $i++) {
        ...
        $stock_number = $sql_arr['stock_num'];
        $date = $sql_arr['time'];
        $data[$i] = array($stock_id, $stock_price, $stock_number, $date);
    }
    $Tcdata = json_encode($data);
    echo $Tcdata;
