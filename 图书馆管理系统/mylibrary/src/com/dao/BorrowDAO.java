...
public class BorrowDAO {
	...
	public int insertBorrow(ReaderForm readerForm, BookForm bookForm,
		...
		java.sql.Date backTime = new java.sql.Date(dateU.getTime());
		...
		String sql = "Insert into tb_borrow (readerid,bookid,borrowTime,backTime,operator) values("
				...
				+ date
				+ "','" + backTime + "','" + operator + "')";
		String sql4 = "UPDATE tb_bookinfo SET stock=stock-1 where id=" + bookForm.getId() + "";
		int falg = conn.executeUpdate(sql);
		conn.executeUpdate(sql4);
			...
			if (rs1.next()) {
				...
				if (ret == 1) {
					String sql2 = "UPDATE tb_borrow SET ifback=1 where id="
							+ id + "";
					String sql3 = "UPDATE tb_bookinfo SET stock=stock+1 where id=(select bookid from tb_borrow where id=" + id + ")";
	...
	public Collection bremind() {
		...
		try {
			while (rs.next()) {
				...
				coll.add(form);
				System.out.println("图书编号：" + rs.getString(3));
			}
		} catch (SQLException ex) {
	...
	public Collection borrowQuery(String strif) {
		...
		ResultSet rs = conn.executeQuery(sql);
		System.out.println("图书借阅信息查询的SQL：" + sql);
		Collection coll = new ArrayList();
		BorrowForm form = null;
		try {
			while (rs.next()) {
