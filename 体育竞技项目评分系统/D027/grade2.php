...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    if($_POST["grade"]<10 && $_POST["grade"] >=-1){
        ...
        if ($con->connect_error)
        {
            ...
            $gradenum = $_COOKIE["userid"] - "000";
            if($gradenum == 1) $gradeid = "grade1";
            else if($gradenum == 2) $gradeid = "grade2";
            else if($gradenum == 3) $gradeid = "grade3";
            else if($gradenum == 4) $gradeid = "grade4";
            else if($gradenum == 5) $gradeid = "grade5";
            $sql1 = "update program set $gradeid = $b , if_grade = if_grade + 1 where program_id = $c and $gradeid = 0";
            $re = $con->query($sql1);
            if(!$re){
                echo "<script>
        ...
        <main class="main-cont content mCustomScrollbar">
            <div class="page-wrap">
                ...
                <section class="page-hd">
                    <header>
                        <h2 class="title">难度评分</h2>
                        <p class="title-description">
                            五个裁判分别打分，去掉一个最高分，去掉一个最低分，剩余分数的平均分几位选手的最后评分。难度分数在0-2范围内。
                        </p>
                    </header>
                    <hr>
                </section>
                ...
                <table class="table table-bordered  mb-15">
                    ...
                    if($_SESSION["start"] == 1){
                        ...
                        if ($con_grade->connect_error)
                        {
                            ...
                            $sql_grade = "SELECT * FROM program NATURAL JOIN item WHERE check_status2 = 1 AND if_grade = 0 ORDER BY group_id ,program_id";
