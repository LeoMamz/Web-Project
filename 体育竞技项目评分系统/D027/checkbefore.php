...
if(!isset($_SESSION["item_name"])) $_SESSION["item_name"] = "所有集体项目项目";
if(!isset($_SESSION["sql_checkbefore"]))
$_SESSION["sql_checkbefore"] = "SELECT * FROM program A NATURAL JOIN item , group_members B
                                  WHERE B.group_code = A.con_id AND item_id >='01' AND item_id <= '02' 
                                  ORDER BY item_id, B.group_code,program_id";
                ...
                <div>
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        ...
                        if($type == 1){
                            ...
                            $a2 = $_POST["conunit"];
                            if($a1 == "所有集体项目"){
                                $a1 = "=\"集体-太极八法五步\" or item_name=\"集体-二十四式太极拳 \"";
                                $_SESSION["item_name"] = "所有集体项目";
                            }else{
                            ...
                            $_SESSION["sql_checkbefore"] = "SELECT * FROM program A NATURAL JOIN item , group_members B
                                  WHERE B.group_code = A.con_id AND item_name $a1 AND (con_unit $a2)
                                  ORDER BY item_id, B.group_code,program_id";
//                            $_SESSION["sql_checkbefore"] = "SELECT * FROM program NATURAL JOIN item where item_name $a1 and con_unit $a2 ORDER BY item_id,group_id,program_id";
                        }else if($type == 2){
                            $a1 =  $_POST['condition'];
                            $random_condition = $a1;
                            $_SESSION["sql_checkbefore"] = "SELECT * FROM program A NATURAL JOIN item , group_members B
                                  WHERE B.group_code = A.con_id AND (item_id >='01' AND item_id <= '02') AND  group_id = '$a1' 
                                  or item_id = '$a1' or item_name = '$a1' or program_id = '$a1' or member_id = '$a1' or member_name = '$a1' 
                                  or group_code = '$a1' or member_unit = '$a1' 
                                  ORDER BY item_id, B.group_code,program_id";
//                            $_SESSION["sql_checkbefore"] = "SELECT * FROM program NATURAL JOIN item where  (item_id >='01' AND item_id <= '02') AND  group_id = '$a1' or item_id = '$a1' or item_name = '$a1' or program_id = '$a1' or con_id = '$a1' or con_name = '$a1' or con_sex = '$a1' or con_unit = '$a1'  ORDER BY item_id, group_id,program_id";
                        }
                    }
                    ?>

                    <form action="checkbefore.php" method="post" class="fl">
                        <div class="fl">
                            <div class="form-cont">
                                <select name="itemname" style="width:auto;" >
                                    <option <?php if($_SESSION["con_unit"] == "所有集体项目") echo"selected";?>>所有集体项目</option>
                                    <?php
                                    $con = new mysqli("localhost", "root", "mamz", "d027");
                                    $con->query("set names 'UTF8'");
                                    if ($con->connect_error)
                                    {
                                        ...
                                        $sql1 = "SELECT item_name FROM item WHERE item_id >= \"01\" AND item_id <= \"02\" ORDER BY item_id";
                                        $re = $con->query($sql1);
                                        if ($re->num_rows > 0) {
                                            //有查到相关记录
                    ...
                    <form action="checkbefore.php" method="post" class="fr input-group">
                ...
                <table class="table table-bordered  mb-15">
                    ...
                    <tbody>
                    <?php
                    $sql_grade = $_SESSION["sql_checkbefore"] ;
                        ...
                        if($sql_grade != ""){
                            ...
                            if(isset($re_grade->num_rows) && $re_grade->num_rows > 0){
                                while($row_grade = $re_grade->fetch_assoc()){
                                    $a = $row_grade["member_id"];
                                    $b = $row_grade["check_status"];
                                    echo "<tr class=\"cen\">";
                                    echo "<td>".$row_grade["program_id"]."</td>";
                                    echo "<td>".$row_grade["group_code"]."</td>";
                                    echo "<td>".$row_grade["item_id"]."</td>";
                                    echo "<td>".$row_grade["item_name"]."</td>";
                                    echo "<td>".$row_grade["member_name"]."</td>";
                                    echo "<td>".$row_grade["member_id"]."</td>";
                                    echo "<td>".$row_grade["member_unit"]."</td>";
                                    if($b == 1){
                                        echo "<td>";
                                        echo "<form action=\"checkbeforedeal.php\" method=\"post\">";
                                        $a = -$a;
                                        echo "<input name=\"check_type\" type='hidden' value=2 />";
                                        echo "<input name=\"member_id\" type='hidden' value=\"$a\" />";
                                        echo "<input type='submit' class=\"btn btn-secondary-outline\" value=\"已检录\" />";
                                        echo "</form>";
                                        echo "</td>";
                                    }else if ($b == 0){
                                        echo "<td>";
                                        echo "<form action=\"checkbeforedeal.php\" method=\"get\">";
                                        echo "<input name=\"check_type\" type='hidden' value=2 />";
                                        echo "<input name=\"member_id\" type='hidden' value=\"$a\" />";
